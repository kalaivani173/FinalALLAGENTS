...
# Payee PSP agent: artifacts here; Java repo default <project_root>/javacoderepo/PayeePSP (override with PAYEE_PSP_REPO or JAVACODEREPO_ROOT)
_root = Path(__file__).resolve().parent
_project_root = _root.parent
_javacoderepo_root = Path(os.environ.get("JAVACODEREPO_ROOT", str(_project_root / "javacoderepo")))
_default_repo = _javacoderepo_root / "PayeePSP"
REPO_PATH = Path(os.environ.get("PAYEE_PSP_REPO", str(_default_repo)))
ARTIFACTS_PATH = _root / "artifacts"
ARTIFACTS_PATH.mkdir(exist_ok=True)

# Agent identity (API responses and orchestrator A2A)
AGENT_NAME = "PAYEE_PSP"
AGENT_ORCHESTRATOR_NAME = "Payee"
PATCH_FILENAME = "payee.patch"
...
# Payee-agent backend port (override with PAYEE_AGENT_PORT if needed)
PORT = int(os.environ.get("PAYEE_AGENT_PORT", "9004"))
