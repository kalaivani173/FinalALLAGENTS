...
from agent.payee_agent import process_manifest
...
app = FastAPI(
    title="Payee PSP AI Agent",
    description="Phase 2: AI agent for Payee PSP – app ready for change after initiation from NPCI Switch.",
)

# CORS for React dev server (e.g. http://localhost:5173)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5176", "http://127.0.0.1:5176", "http://localhost:5173", "http://127.0.0.1:5173"],
...
@app.get("/")
def serve_ui():
    """Serve the Payee-agent UI (React build if present, else static)."""
...
def _patch_content(change_id: str, git_patch_value: str | None) -> str | None:
    """Return patch content. If git_patch_value is a file path (e.g. .../payee.patch), read file."""
