...
# Remitter Bank agent: artifacts here; Java repo default <project_root>/javacoderepo/RemitterBank (override with REMITTER_REPO or JAVACODEREPO_ROOT)
_root = Path(__file__).resolve().parent
_project_root = _root.parent
_javacoderepo_root = Path(os.environ.get("JAVACODEREPO_ROOT", str(_project_root / "javacoderepo")))
_default_repo = _javacoderepo_root / "RemitterBank"
REPO_PATH = Path(os.environ.get("REMITTER_REPO", str(_default_repo)))
ARTIFACTS_PATH = _root / "artifacts"
ARTIFACTS_PATH.mkdir(exist_ok=True)

# Agent identity (API responses and orchestrator A2A)
AGENT_NAME = "REMITTER_BANK"
AGENT_ORCHESTRATOR_NAME = "Remitter"
PATCH_FILENAME = "remitter.patch"
...
# Remitter-agent backend port (override with REMITTER_AGENT_PORT if needed)
PORT = int(os.environ.get("REMITTER_AGENT_PORT", "9003"))
