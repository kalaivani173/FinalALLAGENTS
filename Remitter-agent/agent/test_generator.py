...
def _get_llm():
    ...
    if _llm is None:
        ...
        if not api_key:
            raise ValueError(
                "OPENAI_API_KEY is not set. Set it in .env (Remitter-agent/.env) or as environment variable."
            )
        _llm = ChatOpenAI(
            model=LLM_MODEL,
            temperature=0,
...
PROMPT = ChatPromptTemplate.from_messages([
    ("system",
     "You are a senior Java engineer writing JUnit 5 unit tests for a UPI Remitter Bank (debit/CBS integration) Spring Boot application. "
