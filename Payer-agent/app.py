...
# Load .env from Payer-agent project directory so OPENAI_API_KEY is set regardless of cwd
...
from config import ARTIFACTS_PATH, REPO_PATH, NPCI_PUBLIC_KEY, SKIP_SIGNATURE_VERIFY, PORT, ORCHESTRATOR_URL

async def _notify_orchestrator_status(change_id: str, status: str):
    """POST to orchestrator/a2a/status with payload { changeId, agent: 'Payer', status }. Returns (success, error_msg)."""
    import logging
    import httpx
    _log = logging.getLogger(__name__)
    url = f"{ORCHESTRATOR_URL}/orchestrator/a2a/status"
    payload = {"changeId": change_id, "agent": "Payer", "status": status}
    try:
...
app = FastAPI(
    title="Payer PSP AI Agent",
    description="Phase 2: AI agent for Payer PSP – receives NPCI manifest, updates code/tests/XSD, reports READY.",
)
...
@app.get("/")
def serve_ui():
    """Serve the Payer-agent UI (React build if present, else static)."""
...
def health():
    ...
    return {"status": "ok", "agent": "PAYER_PSP", "openai_configured": key_set}
...
def _patch_content(change_id: str, git_patch_value: str | None) -> str | None:
    """Return patch content. If git_patch_value is a file path (e.g. .../payer.patch), read file."""
    ...
    if s.endswith(".patch") and (s.find("\\") >= 0 or s.find("/") >= 0):
        path = ARTIFACTS_PATH / change_id / "payer.patch"
        if path.exists():
...
async def generate_tests_endpoint(change_id: str):
    ...
    if not patch_content and (path_dir / "payer.patch").exists():
        patch_content = (path_dir / "payer.patch").read_text(encoding="utf-8", errors="replace")
