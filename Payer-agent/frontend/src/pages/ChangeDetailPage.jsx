    ...
    <section className="section detail-page">
      <div className="section-header detail-header">
        <button
          ...
          title="Behind the scenes: Navigates back to the list page."
        >
          ← Back to list
        </button>
          ...
          <div className="detail-actions">
            {(detail.state === 'RECEIVED' || detail.state === 'ACCEPTED') && !detail.artifacts?.gitPatch && (
              <button
                ...
                onClick={generate}
                disabled={generating}
                title="Behind the scenes: POST /agent/status/{id}/generate → process_manifest: RAG + LLM generates Java patch, updates XSD, runs Maven/Gradle tests."
              >
            ...
            {(detail.state === 'RECEIVED' || detail.state === 'ACCEPTED') && detail.artifacts?.gitPatch && (
              <button
                ...
                title="Behind the scenes: POST /agent/status/{id}/generate-tests → LLM generates unit tests, sets TESTS_READY, notifies orchestrator APPLIED."
              >
                {generatingTests ? 'Generating…' : 'Test'}
              </button>
            )}
            {detail.state === 'TESTS_READY' && (
              <button
                type="button"
                className="btn btn-primary"
                onClick={testNow}
                disabled={runningTests}
                title="Behind the scenes: POST /agent/status/{id}/run-tests → sets state TESTED, notifies orchestrator TESTED."
              >
                {runningTests ? 'Updating…' : 'TEST NOW'}
              </button>
            )}
            {detail.state === 'TESTED' && detail.artifacts?.gitPatch && (
              <button
                ...
                onClick={approve}
                disabled={approving}
                title="Behind the scenes: POST /agent/status/{id}/approve → sets APPLIED, notifies orchestrator READY via POST /orchestrator/a2a/status."
              >
            ...
            {(detail.state === 'READY_FOR_DEPLOYMENT' || detail.state === 'APPLIED') && (
              <button
                ...
                title="Behind the scenes: POST /agent/status/{id}/deploy → sets state DEPLOYED (UI status)."
              >
