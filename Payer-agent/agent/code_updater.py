...
from config import REPO_PATH, ARTIFACTS_PATH
...
def apply_code(manifest: dict) -> str:
    """Apply manifest change to Payer PSP local repo. Returns patch content (also written to payer.patch). No git required."""
    cid = manifest["changeId"]
    path = ARTIFACTS_PATH / cid
    path.mkdir(parents=True, exist_ok=True)
    patch_file = path / "payer.patch"

    if not REPO_PATH.exists():
        placeholder = f"# Change {cid}\n# Place Payer PSP repo at {REPO_PATH} and re-run.\n"
        patch_file.write_text(placeholder, encoding="utf-8", errors="replace")
        return placeholder

    docs, vector_store = _get_vector_store()
    if not docs:
        placeholder = f"# Change {cid}\n# No Java/XSD files in {REPO_PATH}. Add Payer PSP code and re-run.\n"
        patch_file.write_text(placeholder, encoding="utf-8", errors="replace")
        return placeholder
