...
from agent.beneficiary_agent import process_manifest
...
app = FastAPI(
    title="Beneficiary Bank AI Agent",
    description="Phase 2: AI agent for Beneficiary Bank – updates credit/CBS integration logic and validators per change.",
)
...
@app.get("/")
def serve_ui():
    """Serve the Beneficiary-agent UI (React build if present, else static)."""
...
def _patch_content(change_id: str, git_patch_value: str | None) -> str | None:
    """Return patch content. If git_patch_value is a file path (e.g. .../beneficiary.patch), read file."""
