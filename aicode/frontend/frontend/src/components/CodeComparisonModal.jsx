...
import { FileCode, CheckCircle2, XCircle, RefreshCw, FileText, ChevronRight, Download, Loader2, Upload } from 'lucide-react'
import { changeRequestAPI, createAndStoreManifest, getManifestDownloadUrl, specApprove, uploadArtifact } from '@/lib/api'
...
export default function CodeComparisonModal({ open, onOpenChange, changeRequest, onStatusUpdated, showApproveReject = true }) {
  ...
  const handleApprove = async () => {
    setSubmitting(true)
    try {
      // Upload optional product note before approving (manual override; skips agent generation)
      if (productNoteFile && changeRequest?.changeId) {
        try {
          await uploadArtifact(changeRequest.changeId, 'productNotes', productNoteFile)
        } catch (err) {
        ...
        {!changeRequest?.loading && canApproveReject && (
          <div className="px-6 py-4 border-t bg-slate-50 flex-shrink-0">
            <div className="space-y-3">
              <div>
                <Label className="text-sm font-medium">Product note upload <span className="text-slate-500 font-normal">(optional)</span></Label>
                <p className="text-xs text-slate-500 mt-0.5 mb-1">PDF, Markdown, or DOCX. If omitted, manifest creation will auto-generate one.</p>
                <FileUpload
                  accept=".pdf,.md,.docx"
              ...
              <div className="flex justify-end gap-3">
                ...
                <Button
                  ...
                  title="Behind the scenes: PATCH change-requests/{id} with Rejected; no manifest created."
                >
                ...
                <Button
                  ...
                  title="Behind the scenes: spec/approve → PATCH change-requests Approved → backend creates signed manifest."
                >
