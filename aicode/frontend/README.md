...
> **Where to run:** Always run `npm run dev` from **this folder** (`aicode/frontend`), not from `frontend/frontend`. The app uses `./src` here; the nested `frontend/frontend` is a duplicate and was synced with this UI—use this folder as the single source of truth for edits.
