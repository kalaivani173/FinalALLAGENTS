...
export default function DeveloperChangeRequestsTab({ isActive }) {
  ...
  const handleDeploy = async (changeId) => {
    ...
    try {
      ...
      if (data?.success) {
        const fileCount = (data.deployedFiles || []).length
        const gcp = data?.gitCommitPush
        let detail = `${fileCount} file(s) deployed to javacoderepo.`
        if (gcp && typeof gcp === 'object') {
          if (gcp.pushed) {
            detail += ` Git: pushed to ${gcp.remote || 'origin'}/${gcp.branch || 'branch'}${gcp.commitHash ? ` (${String(gcp.commitHash).slice(0, 7)})` : ''}.`
          } else if (gcp.ok) {
            detail += ` Git: ${gcp.message || 'committed'}${gcp.commitHash ? ` (${String(gcp.commitHash).slice(0, 7)})` : ''}.`
          } else {
            detail += ` Git: ${gcp.message || 'push failed'}${gcp.errorCode ? ` [${gcp.errorCode}]` : ''}.`
          }
        }
        toast.success('Deployed', detail)
        fetchChangeRequests()
      } else {
      ...
      <Card className="w-full min-w-0">
        ...
        <CardContent className="w-full min-w-0">
          {loading ? (
            ...
            <div className="border rounded-lg w-full min-w-0 overflow-x-hidden">
              <Table className="w-full table-fixed">
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[11%] min-w-0">Change Request ID</TableHead>
                    <TableHead className="w-[24%] min-w-0">Description</TableHead>
                    <TableHead className="w-[12%] min-w-0">Change Type</TableHead>
                    <TableHead className="w-[11%] min-w-0">Received Date</TableHead>
                    <TableHead className="w-[14%] min-w-0">Current Status</TableHead>
                    <TableHead className="w-[11%] min-w-0">Updated On</TableHead>
                    <TableHead className="w-[17%] min-w-0">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                    ...
                    >
                      <TableCell className="min-w-0 overflow-hidden">
                        <button
                          ...
                          className="text-[#2F888A] hover:text-[#2F888A]/90 hover:underline font-medium cursor-pointer truncate block text-left w-full"
                        >
                          {request.changeId}
                        </button>
                      </TableCell>
                      ...
                      <TableCell className="min-w-0 truncate">{formatChangeType(request.changeType)}</TableCell>
                      <TableCell className="min-w-0 whitespace-nowrap truncate" title={formatDate(request.receivedDate)}>{formatDate(request.receivedDate)}</TableCell>
                      <TableCell className="min-w-0">
                        <span
                          className={`inline-block px-2 py-1 rounded-full text-xs font-medium truncate max-w-full ${
                            STATUS_COLORS[request.currentStatus] || 'bg-gray-100 text-gray-800'
                          }`}
                          title={request.currentStatus}
                        >
                          {request.currentStatus}
                        </span>
                      </TableCell>
                      <TableCell className="min-w-0 whitespace-nowrap truncate" title={request.updatedOn || formatDate(request.receivedDate)}>{request.updatedOn || formatDate(request.receivedDate)}</TableCell>
                      <TableCell className="min-w-0">
