          ...
          {showProductTabs ? (
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <div className="grid gap-6 lg:grid-cols-[260px_1fr] lg:items-stretch">
                ...
                {/* Main content — elevated glass panel; w-full min-w-0 so no horizontal scrollbar */}
                <div className="min-w-0 w-full max-w-full rounded-2xl border border-white/25 bg-white/20 backdrop-blur-xl shadow-2xl p-4 sm:p-6 ring-1 ring-white/10 overflow-x-hidden" data-portal-glass>
                  {/* Top nav (mobile) */}
                  ...
                  <TabsContent value="change-requests" className="mt-0 w-full min-w-0 data-[state=active]:animate-in data-[state=active]:fade-in-0 data-[state=active]:slide-in-from-bottom-1">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <div className="grid gap-6 lg:grid-cols-[260px_1fr] lg:items-stretch">
                ...
                <div className="min-w-0 w-full max-w-full rounded-2xl border border-white/25 bg-white/20 backdrop-blur-xl shadow-2xl p-4 sm:p-6 ring-1 ring-white/10 overflow-x-hidden" data-portal-glass>
                  ...
                  <TabsContent value="create-request" className="mt-0 w-full min-w-0 data-[state=active]:animate-in data-[state=active]:fade-in-0 data-[state=active]:slide-in-from-bottom-1">
                    <ChangeRequestsTab />
                  </TabsContent>
                </div>
              </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <div className="grid gap-6 lg:grid-cols-[260px_1fr] lg:items-stretch">
                ...
                <div className="min-w-0 w-full max-w-full rounded-2xl border border-white/25 bg-white/20 backdrop-blur-xl shadow-2xl p-4 sm:p-6 ring-1 ring-white/10 overflow-x-hidden" data-portal-glass>
                  ...
                  <TabsContent value="developer-requests" className="mt-0 w-full min-w-0 data-[state=active]:animate-in data-[state=active]:fade-in-0 data-[state=active]:slide-in-from-bottom-1">
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (changeType === 'field-addition') {
        ...
        const buildXmlPath = (api, element) => {
          const apiTrimmed = (api || '').trim()
          const elemTrimmed = (element || '').trim()
          if (!apiTrimmed || !elemTrimmed) return ''

          // Special-case: in ReqPay XSD, Device is under Payer, not directly under ReqPay.
          // If user selects "Device" for ReqPay, map it to ReqPay.Payer.Device automatically.
          if (apiTrimmed === 'ReqPay' && elemTrimmed === 'Device') {
            return `${apiTrimmed}.Payer.Device`
          }

          return `${apiTrimmed}.${elemTrimmed}`
        }
        ...
        const additions = (fieldAdditions || [])
          .map((r) => {
            ...
            return {
              xmlPath: buildXmlPath(apiName, element),
              elementName: element,
                      ...
                      <div key={row.id} className="border rounded-lg bg-white p-4 space-y-4">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          {fieldAdditions.length > 1 && (
                            <Button type="button" variant="ghost" onClick={() => removeFieldRow(row.id)}>
                              Remove
                            </Button>
                          )}
                        </div>
            ...
            <div className="space-y-4 border-t pt-6 mt-6">
              <div className="p-4 rounded-lg bg-primary/5 border border-primary/20">
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  <Button type="button" variant="default" size="sm" onClick={() => onSubmitted?.()}>
...
function ByChangeRequestSection() {
  ...
  const PAGE_SIZE = 20
  const [orderedChangeIds, setOrderedChangeIds] = useState([])
  ...
  const fetchStatus = async () => {
    ...
    try {
      ...
      setCrById(byId)
      setOrderedChangeIds(list.map((cr) => cr.changeId).filter(Boolean))
    } catch {
